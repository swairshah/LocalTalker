...
final class AudioCaptureSession {
    ...
    func start() throws {
        ...
        print("LocalTalker: Pre-VP input format: \(Int(preVPFormat.sampleRate))Hz, \(preVPFormat.channelCount)ch")
        ...
        if voiceProcessingEnabled {
            do {
                ...
                print("LocalTalker: Voice processing (AEC) enabled, AGC off, ducking minimized")
            } catch {
                print("LocalTalker: Warning — voice processing unavailable: \(error)")
            }
        }

        // ── Step 2: Query format AFTER VP is configured ──
        let vpioFormat = inputNode.outputFormat(forBus: 0)
        print("LocalTalker: Post-VP input format: \(Int(vpioFormat.sampleRate))Hz, \(vpioFormat.channelCount)ch")
        ...
        inputNode.installTap(onBus: 0, bufferSize: bufferSize, format: nil) { [weak self] buffer, _ in
            ...
            if tapCallCount <= 10 {
                ...
                print("LocalTalker: [tap #\(tapCallCount)] \(Int(buffer.format.sampleRate))Hz/\(buffer.format.channelCount)ch frames=\(frameLen) ch0_RMS=\(String(format: "%.6f", ch0RMS))")
            }
        ...
        print("LocalTalker: Audio capture started (\(Int(vpioRate))Hz → 16kHz)\(aecStatus)\(chInfo)")
    }
    ...
    private func convertAndDeliver(
        ...
        if status == .haveData {
            if tapCount <= 5 {
                ...
                print("LocalTalker: [tap #\(tapCount)] converted=\(convertedBuffer.frameLength)frames outRMS=\(String(format: "%.6f", outRMS))")
            }
            deliverBuffer(convertedBuffer)
        } else if tapCount <= 5 {
            print("LocalTalker: [tap #\(tapCount)] conversion failed: \(error?.localizedDescription ?? "unknown")")
        }
    }
    ...
    func stop() {
        ...
        print("LocalTalker: Audio capture stopped")
    }
