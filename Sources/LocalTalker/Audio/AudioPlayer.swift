...
final class AudioPlayer {
    ...
    func attach(to engine: AVAudioEngine) {
        ...
        print("LocalTalker: AudioPlayer attached to engine (mixer: \(Int(mixerFormat.sampleRate))Hz, \(mixerFormat.channelCount)ch)")
    }
    ...
    private func playViaEngine(audioData: Data) async throws {
        ...
        // CRITICAL: reset converter between independent audio segments.
        // Without this, the converter's internal resampling state carries
        // residual samples from the previous conversion, prepending stale
        // audio from the last turn to the current one.
        converter.reset()

        // Provide input exactly once. The converter may call the block
        // multiple times to fill the output buffer — returning the same
        // buffer again would duplicate audio.
        ...
        guard status == .haveData || status == .endOfStream else {
            if let err = conversionError {
                print("LocalTalker: Audio conversion error: \(err)")
            }
            throw PlayerError.invalidAudioData
        }

        print("🔈 [AUDIO] Playing \(outputBuffer.frameLength) frames (\(String(format: "%.2f", Double(outputBuffer.frameLength) / outFormat.sampleRate))s) from \(sampleCount) input samples")
        ...
        await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
            ...
            // Verify node is still attached to a running engine
            guard let engine = attachedEngine, engine.isRunning, node.engine != nil else {
                playbackContinuation = nil
                lock.unlock()
                previousCont?.resume()
                continuation.resume()
            ...
            // Double-check node is still attached to a running engine before playing
    ...
    private func playViaFFPlay(audioData: Data) async throws {
        ...
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("localtalker-playback-\(UUID().uuidString).raw")
